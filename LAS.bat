...
"%REQ_DIR%\vc_redist.x64.exe" /quiet /norestart || (echo [ERRO] Falha na instalação do VC Redist && pause && exit /b)
...
call %COMPOSER_PATH% install || (echo [ERRO] Composer Install falhou && pause && exit /b)
call npm install || (echo [ERRO] NPM Install falhou && pause && exit /b)
call npm run build || (echo [ERRO] NPM Build falhou && pause && exit /b)

echo.
echo ====== Etapa 7: Configurando Laravel ======
copy ".env.example" ".env" /Y || (echo [ERRO] Falha ao copiar .env && pause && exit /b)
call php artisan key:generate || (echo [ERRO] Falha ao gerar chave Laravel && pause && exit /b)
